...
<xsl:stylesheet xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <xsl:template match="/">
    <Document locale="{$isoLangId}">
      ...
      <xsl:apply-templates select="gmd:MD_Metadata|gmi:MI_Metadata|*[@gco:isoType='gmd:MD_Metadata']"
                           mode="metadata"/>
  ...
  <xsl:template match="*" mode="metadata">
    ...
    <xsl:for-each select="gmd:identificationInfo//gmd:MD_DataIdentification|
      ...
      <xsl:for-each select="//gmd:MD_Keywords">
        ...
        <xsl:variable name="listOfKeywords" select="gmd:keyword/*"/>
        ...
        <xsl:for-each select="gmd:thesaurusName/gmd:CI_Citation">
          ...
          <xsl:if test="gmd:title/*/text() != ''">
            <Field name="thesaurusName"
                   string="{gmd:title/*/text()}"
                   store="true" index="true"/>
          </xsl:if>
