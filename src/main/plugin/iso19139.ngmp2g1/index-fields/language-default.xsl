...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
   ...
   <xsl:variable name="md" select="/(gmd:MD_Metadata|gmi:MI_Metadata|*[@gco:isoType='gmd:MD_Metadata'])"/>
     
    <Documents>
      <xsl:for-each select="$md/gmd:locale/gmd:PT_Locale">
      ...
      <xsl:if test="$md/gmd:locale/gmd:PT_Locale
                     and count($md/gmd:locale/gmd:PT_Locale/gmd:languageCode/gmd:LanguageCode[@codeListValue = $isoDocLangId]) = 0">
        <xsl:call-template name="document">
  ...
  <xsl:template name="document">
    ...
    <Document locale="{$isoLangId}">
      ...
      <xsl:variable name="title"
                    select="$md/gmd:identificationInfo/*/gmd:citation//gmd:title//gmd:LocalisedCharacterString[@locale=$poundLangId]"/>
        ...
        <xsl:for-each select="gmd:onLine/gmd:CI_OnlineResource">
          ...
          <xsl:if test="starts-with($protocol,'OGC:WMS') and
            ...
            <Field name="link" string="{concat($title, '|', $desc, '|',
              '../../srv/en/google.kml?uuid=', /(gmd:MD_Metadata|gmi:MI_Metadata)/gmd:fileIdentifier/gco:CharacterString, '&amp;layers=', $title,
