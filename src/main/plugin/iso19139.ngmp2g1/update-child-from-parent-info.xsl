...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:srv="http://www.isotc211.org/2005/srv"
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="/root/update/parent/(gmd:MD_Metadata|gmi:MI_Metadata)"/>
  </xsl:template>

  <!-- ================================================================= -->

  <xsl:template match="/root/update/parent/(gmd:MD_Metadata|gmi:MI_Metadata)">
     
     <xsl:variable name="parent" select="/root/update/parent/(gmd:MD_Metadata|gmi:MI_Metadata)"/>
     <xsl:variable name="child" select="/root/child/(gmd:MD_Metadata|gmi:MI_Metadata)"/>
     
    <xsl:copy>
      <xsl:copy-of select="$child/gmd:fileIdentifier"/>
      <xsl:copy-of select="$child/gmd:language|gmd:characterSet"/>
      <xsl:copy-of select="$child/gmd:parentIdentifier"/>
      <xsl:copy-of select="$child/gmd:hierarchyLevel"/>
      <xsl:copy-of select="$child/gmd:hierarchyLevelName"/>
      ...
      <xsl:copy-of select="$child/gmd:dataSetURI"/>
      ...
      <gmd:identificationInfo>
        <xsl:for-each select="$child/gmd:identificationInfo/*">
          <xsl:copy>
            <xsl:copy-of select="@*"/>
            ...
            <xsl:call-template name="process">
              ...
              <xsl:with-param name="name"
                              select="$parent/gmd:identificationInfo/*/gmd:pointOfContact"/>
              <xsl:with-param name="mode" select="$updateMode"/>
              <xsl:with-param name="subLevel" select="true()"/>
            </xsl:call-template>

            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:resourceMaintenance"/>
            <xsl:copy-of select="gmd:graphicOverview"/>
            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:resourceFormat"/>

            <xsl:call-template name="process">
              <xsl:with-param name="update" select="$gmd-descriptiveKeywords"/>
              <xsl:with-param name="name" select="$parent/gmd:identificationInfo/*/gmd:descriptiveKeywords"/>
              <xsl:with-param name="mode" select="$updateMode"/>
              <xsl:with-param name="subLevel" select="true()"/>
            </xsl:call-template>

            <!-- FIXME / TO BE DISCUSS following sections are replaced (excepts AggregationInfo preserved). -->
            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:resourceSpecificUsage"/>
            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:resourceConstraints"/>
            <xsl:copy-of select="gmd:aggregationInfo"/>
            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:spatialRepresentationType"/>
            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:spatialResolution"/>
            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:langage"/>
            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:characterSet"/>
            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:topicCategory"/>
            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:environmentDescription"/>

            <xsl:call-template name="process">
              <xsl:with-param name="update" select="$gmd-extent"/>
              <xsl:with-param name="name"
                              select="$parent/gmd:identificationInfo/*/gmd:extent"/>
              <xsl:with-param name="mode" select="$updateMode"/>
              <xsl:with-param name="subLevel" select="true()"/>
            </xsl:call-template>

            <!-- FIXME / TO BE DISCUSS following sections are replaced/preserved  -->
            <xsl:copy-of select="$parent/gmd:identificationInfo/*/gmd:supplementalInformation"/>
      ...
      <!-- FIXME / TO BE DISCUSS following sections are preserved -->
      <xsl:copy-of select="$child/gmd:series"/>
      <xsl:copy-of select="$child/gmd:describes"/>
      <xsl:copy-of select="$child/gmd:propertyType"/>
      <xsl:copy-of select="$child/gmd:featureType"/>
      <xsl:copy-of select="$child/gmd:featureAttribute"/>
    ...
    <xsl:variable name="childElement">
      <xsl:choose>
        <xsl:when test="$subLevel=true()">
          <xsl:copy-of select="$child/gmd:identificationInfo/*/*[name(.)=name($name)]"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:copy-of select="$child/*[name(.)=name($name)]"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
