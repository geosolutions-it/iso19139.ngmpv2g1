...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="iso19139.ngmp2g1schema" select="/root/gui/schemas/iso19139.ngmp2g1"/>
  <xsl:variable name="iso19139.ngmp2g1labels" select="$iso19139.ngmp2g1schema/labels"/>
  <xsl:variable name="iso19139.ngmp2g1codelists" select="$iso19139.ngmp2g1schema/codelists"/>
  <xsl:variable name="iso19139.ngmp2g1strings" select="$iso19139.ngmp2g1schema/strings"/>

  <!-- Match codelist values. 
        Must use iso19139.ngmp2g1 because some 19139 codelists are extended in ngmp2g1
        If the codelist exists in iso19139.ngmp2g1 then use that otherwise use iso19139 codelists
  ...
  -->
  <xsl:template mode="mode-iso19139" priority="30000" match="*[*/@codeList and $schema='iso19139.ngmp2g1' and name()!='gmd:dateType']">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="codelists" select="$iso19139.ngmp2g1codelists" required="no"/>
    ...
    <xsl:variable name="elementName" select="name()"/>
		...
		<!-- check iso19139.ngmp2g1 first, then fall back to iso19139 -->
		<xsl:variable name="listOfValues" as="node()">
			<xsl:variable name="ngmp2g1List" as="node()" select="gn-fn-metadata:getCodeListValues($schema, name(*[@codeListValue]), $codelists, .)"/>
			<xsl:choose>
				<xsl:when test="count($ngmp2g1List/*)=0"> <!-- do iso19139 -->
					<xsl:copy-of select="gn-fn-metadata:getCodeListValues('iso19139', name(*[@codeListValue]), $iso19139codelists, .)"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:copy-of select="$ngmp2g1List"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
	...
	<!--
    Take care of enumerations. Same as for codelists, check iso19139.ngmp2g1
  ...
  -->
  <xsl:template mode="mode-iso19139" priority="30000" match="*[gn:element/gn:text and $schema='iso19139.ngmp2g1']">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="codelists" select="$iso19139.ngmp2g1codelists" required="no"/>
		...
		<!-- check iso19139.ngmp2g1 first, then fall back to iso19139 -->
		<xsl:variable name="listOfValues" as="node()">
			<xsl:variable name="ngmp2g1List" as="node()" select="gn-fn-metadata:getCodeListValues($schema, name(), $codelists, .)"/>
			<xsl:choose>
				<xsl:when test="count($ngmp2g1List/*)=0"> <!-- do iso19139 -->
  ...
  -->
  <xsl:template mode="mode-iso19139" priority="2000" match="gmd:CI_Date/gmd:date[$schema='iso19139.ngmp2g1']">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="codelists" select="$iso19139.ngmp2g1codelists" required="no"/>
    ...
    <xsl:variable name="dateTypeElementRef"
		...
		<xsl:variable name="listOfValues" as="node()">
			<xsl:variable name="ngmp2g1List" as="node()" select="gn-fn-metadata:getCodeListValues($schema, 'gmd:CI_DateTypeCode', $codelists, .)"/>
			<xsl:choose>
				<xsl:when test="count($ngmp2g1List/*)=0"> <!-- do iso19139 -->
